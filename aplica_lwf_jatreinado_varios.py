...
def Roda_aplica_lwf( namecase, ilayer, csvOld, csvNew, taskData, taskModel, inputCycle ):
  ...
  elif taskData == "Russia":
    taskPath = False
    infoTestFold = False
    images_path = '/mnt/brics_data/public/datasets/Russia/russia/raw/images/'

    # Russia - /mnt/brics_data/public/datasets/Russia/russia/raw
    # Indonesia* - /mnt/brics_data/public/datasets/Indonesia/indonesia/raw
    # Caxias* - /mnt/brics_data/public/datasets/Caxias/caxias/raw
    # Montgomey - /mnt/brics_data/public/datasets/Montgomery/montgomery/raw
    # Índia - /mnt/brics_data/public/datasets/India/india/raw
  ...
  if taskData == 'Russia':
    datas, labels, namesImg = Load_Names_OOD_1Label( images_path, True )
  ################################################################################################
  ...
  if ( calcula_roc ):
    ...
    for imodel in range( nkfolds ):  
      ...
      if ( taskData == 'Stanford' ):
        ...
        datalwf, labellwf, namelwf = Define_Data_Fold_Russia( datas, labels, namesImg )                   
  ...
  resultados = {'Class_Train': Class_Train, 'Class_Val': Class_Val, 'Class_Test': Class_Test, 'Class_TrainVal': Class_TrainVal, 
                ...
                'labels_last_val': labels_last, 'names_last_val': names_last, 
                  ...
                  'names_last_train': names_last_train  }

  a_file = open( results_file, "wb" )
  pickle.dump( resultados, a_file )
  a_file.close()

  Data_csv['Caso'] = namecase + sufixo
...
namecases.append('2WganSoFC_3ManausAll_4SCasaCycleAll')
namecases.append('2WganSoFC_3ManausAll_4SCasaCycleManaus')
namecases.append('2WganSoFC_3ManausAll_4SCasaCycleShenzhen')

ilayers = [1, 2, 3]
taskDatas = ['Manaus','Shenzhen','SantaCasa']
taskModel = 'SantaCasa'

csvOld = ''
csvNewPrefix = pathcsv + 'ResultsSC_'
count = 0
for namecase in namecases:
    if "CycleShenzhen" in namecase:
        inputCycle = 'ShenzhenToSantaCasa'
    elif "CycleManaus" in namecase:
        inputCycle = 'ManausToSantaCasa'
    elif "CycleAll" in namecase:
        inputCycle = 'AllToSantacasa'
    for taskData in taskDatas:
      for ilayer in ilayers:
        count += 1
        if count > 1:
          csvOld = csvNew
        csvNew = csvNewPrefix + str(count) + '.csv'
        Roda_aplica_lwf( namecase, ilayer, csvOld, csvNew, taskData, taskModel, inputCycle )